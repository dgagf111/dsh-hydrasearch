  ...
  factory: (require) => {
    ...
    const I18N = {
      zh: {
        ...
        keyFromConfig: '来自插件配置',
        keyFallback: '回退到环境变量 / 本地文件',
        keyNone: '未配置',
        keyPlaceholder: (has) => (has ? '已配置（留空则不修改）' : '粘贴 API Key'),
        keySave: '写入凭据中心',
        keyClear: '清除',
        keyWritable: '保存后立即生效，无需重启。',
      ...
      en: {
        ...
        keyFromConfig: 'From the plugin config',
        keyFallback: 'Falling back to the environment / local file',
        ...
        keyClear: 'Clear',
        keyWritable: 'Applies immediately, no restart.',
        ...
        autoKeySaveEnv: 'Write to the skill .env',
    ...
    function pickEditable(value) {
      const out = { priority: value.priority.slice(), failover: value.failover, fetchBackend: value.fetchBackend }
    ...
    function BackendSection(props) {
      ...
      const cfg = draft[id]
      const keyBadge = !cfg.enabled
        ? { cls: ' dshhs-badgeOff', text: t.keyNone }
        : credential.configured
          ? { cls: ' dshhs-badgeOk', text: t.keyConfigured }
          : credential.fromConfig
            ? { cls: '', text: t.keyFromConfig }
            : credential.hasFallback
              ? { cls: ' dshhs-badgeOk', text: t.keyFallback }
              : { cls: ' dshhs-badgeWarn', text: t.keyNone }
            ...
            jsx.jsxs('div', { className: 'dshhs-row', children: [
              ...
              credential.configured
                ? jsx.jsx('button', {
                    type: 'button',
                    className: 'dshhs-btn',
                    disabled: busy !== null || !credential.writable,
                    onClick: onClearKey,
                    children: t.keyClear,
                  })
                : null,
    ...
    function HydraSearchCard(props) {
      ...
      const adoptKey = react.useCallback(async (target) => {
        if (autoKey === null) return
        setBusy('adopt')
        const result = await bridge('/key-adopt', { key: autoKey, target })
        setBusy(null)
        if (!result.ok) {
        ...
        setAutoKey(null)
        const stored = result.value.stored === 'skill-env' ? result.value.path : result.value.ref
        setNotice({ kind: 'ok', text: t.autoKeySaved(stored) })
        void load()
      }, [autoKey, load, t])
      ...
      const credentialFor = (id) => Object.assign(
        { configured: false, writable: false, available: false, fromConfig: false, hasFallback: false },
        meta.credentials[id],
        { hasFallback: id === 'tinyfish' ? meta.env.tinyfish.hasFallbackKey : meta.env.anysearch.hasFallbackKey },
      )
      ...
      const body = jsx.jsxs('div', { className: 'dshhs-body', children: [
        ...
        jsx.jsxs('div', { className: 'dshhs-section', children: [
          ...
          jsx.jsxs('div', { className: 'dshhs-field', children: [
            jsx.jsx('span', { className: 'dshhs-label', children: t.fetchBackend }),
            jsx.jsx('select', {
              className: 'dshhs-select',
              value: draft.fetchBackend,
              disabled: busy !== null,
              onChange: (event) => setDraft((prev) => Object.assign({}, prev, { fetchBackend: event.target.value })),
              children: [
        ...
        autoKey === null ? null : jsx.jsxs('div', { className: 'dshhs-warn', children: [
          ...
          jsx.jsxs('div', { className: 'dshhs-row', style: { marginTop: '6px' }, children: [
            jsx.jsx('button', {
              ...
              disabled: busy !== null,
              onClick: () => void adoptKey(undefined),
              children: t.autoKeySaveCred,
            }),
            jsx.jsx('button', {
              type: 'button', className: 'dshhs-btn',
              disabled: busy !== null,
              onClick: () => void adoptKey('env-file'),
              children: t.autoKeySaveEnv,
            }),
          ] }),
        ] }),
