  ...
  factory: (require) => {
    ...
    const I18N = {
      zh: {
        ...
        fetchImageLinks: '抓取时同时返回图片链接',
        fetchPerUrlTimeoutMs: '单 URL 抓取超时（毫秒，0=不发送）',
        fetchTtlSeconds: '抓取缓存容忍度（秒，-1=不发送，0=强制实时）',
        verbose: '记录每次搜索/抓取日志',
      ...
      en: {
        ...
        fetchImageLinks: 'Also return the page\u2019s image links',
        fetchPerUrlTimeoutMs: 'Per-URL fetch timeout (ms, 0 = omit)',
        fetchTtlSeconds: 'Fetch cache tolerance (s, -1 = omit, 0 = force live)',
    ...
    const TINYFISH_FIELDS = [
      ...
      { key: 'fetchImageLinks', kind: 'toggle', label: 'fetchImageLinks' },
      { key: 'fetchPerUrlTimeoutMs', kind: 'number', label: 'fetchPerUrlTimeoutMs' },
      { key: 'fetchTtlSeconds', kind: 'number', label: 'fetchTtlSeconds' },
