 ...
 *
 *   - `tinyfish`    — TinyFish only, no failover (pin this to bypass the chain);
 *   - `anysearch`   — AnySearch only, no failover;
 *   - `hydrasearch` — the failover chain, walking `config.priority` in order.
 *
 * The chain is what `web.searchProvider` points at by default. Registering the
 * two backends as first-class providers as well means an operator can pin a
 * single backend by editing one config value, without losing the chain.
 *
 * ## Priority
 *
...
/** Backend ids. These are provider ids AND `priority` entries. */
export const TINYFISH_ID = 'tinyfish'
export const ANYSEARCH_ID = 'anysearch'

/** Id of the failover-chain provider. */
...
export const TinyfishConfig = z.object({
  ...
  /** API key. Empty falls through to `$TINYFISH_API_KEY`, then `~/.tinyfish/config.json`. */
  apiKey: z.string().role('secret').default(''),
  /** Credential-center reference consulted before the other key sources. */
  apiKeyEnv: z.string().default(TINYFISH_API_KEY_REF),
...
export const AnysearchConfig = z.object({
  ...
  enabled: z.boolean().default(true),
  /**
   * API key. Empty falls through to `$ANYSEARCH_API_KEY`, then the skill's
   * `.env`. A blank key is NOT a failure: AnySearch allows anonymous access at a
   * lower rate limit.
   */
  apiKey: z.string().role('secret').default(''),
  /** Credential-center reference consulted before the other key sources. */
  apiKeyEnv: z.string().default(ANYSEARCH_API_KEY_REF),
  /** API host. Empty uses `$ANYSEARCH_API_BASE_URL`, then the public host. */
...
export class BackendRuntime {
  ...
  /**
   * Resolve the effective key for one operation, per call and never cached, so a
   * key written from the settings card reaches the next request without a
   * restart — the per-operation read the credentials seam documents.
   *
  ...
  async key() {
    ...
    if (credentials !== undefined && cfg.apiKeyEnv.length > 0) {
      try {
        ...
        // A credentials provider that cannot answer must not fail the search:
        // the configured key, environment, and CLI/skill sources are still tried.
      }
    }
    return this.id === TINYFISH_ID ? resolveTinyfishKey(cfg.apiKey) : resolveAnysearchKey(cfg.apiKey)
  }

  /**
   * Whether this backend can run. Cheap and network-free, per the seam's
   * `available()` contract. AnySearch is available anonymously; TinyFish is not,
   * because every TinyFish endpoint requires a key.
   *
  ...
  available() {
    ...
    if (this.id === TINYFISH_ID) {
      ...
      if (cfg.apiKey.length > 0) return true
      return resolveTinyfishKey('').length > 0
    }
...
export class HydraSearchProvider {
  ...
  async fetch(request, signal) {
    const cfg = this.getConfig()
    const pinned = cfg.fetchBackend !== 'auto' && DEFAULT_PRIORITY.includes(cfg.fetchBackend)
      ? [this.backendFor(cfg.fetchBackend)]
      : this.chain()
    const attempts = []
    let firstFailure = null
...
/**
 * A single-backend provider, registered so an operator can pin one backend by
 * setting `web.searchProvider` (or `web.fetchProvider`) to its id.
 */
export class SingleBackendProvider {
  /**
   * @param id - the backend id, which is also this provider's id.
   * @param backend - the {@link BackendRuntime} to delegate to.
   */
  ...
  /** @returns whether the backend is enabled and locally usable. */
  available() {
   ...
   * @param request - the seam's search request.
   * @param signal - cancellation signal.
   * @returns the backend's search result.
   */
  async search(request, signal) {
    const result = await this.backend.search(request, signal)
   ...
   * @param request - the seam's fetch request.
   * @param signal - cancellation signal.
   * @returns the backend's fetch result in the seam's shape.
   */
  async fetch(request, signal) {
    return toFetchResult(await this.backend.fetch(request, signal))
  }
}
...
export function makeBridgeRoutes(deps) {
  const { settings, getCredentials, getConfig, probeSearch, probeBackend, subDomains, adoptKey } = deps
  ...
  /**
   * Credential status for one reference. Uses the credential center when the
   * service is mounted, and the configured/env/file sources otherwise, so the
   * card shows the truth for whichever source is actually supplying the key.
   */
  const keyStatus = async (ref, configuredKey, envVar) => {
    ...
    return {
      ...
      fromConfig: typeof configuredKey === 'string' && configuredKey.length > 0,
    }
  }
      ...
      handler: async (req, res) => {
        ...
        writeJson(res, 200, {
          ...
          value: {
            ...
            credentials: {
              tinyfish: await keyStatus(TINYFISH_API_KEY_REF, cfg.tinyfish.apiKey, TINYFISH_API_KEY_ENV),
              anysearch: await keyStatus(ANYSEARCH_API_KEY_REF, cfg.anysearch.apiKey, ANYSEARCH_API_KEY_ENV),
            },
            env: {
              tinyfish: {
                envVar: TINYFISH_API_KEY_ENV,
                hasEnvKey: Boolean(process.env[TINYFISH_API_KEY_ENV]),
                hasFallbackKey: resolveTinyfishKey('').length > 0,
              },
              anysearch: {
                envVar: ANYSEARCH_API_KEY_ENV,
                hasEnvKey: Boolean(process.env[ANYSEARCH_API_KEY_ENV]),
                hasFallbackKey: resolveAnysearchKey('').length > 0,
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          writeJson(res, 200, { ok: true, value: { ref, set: false } })
        } catch (error) {
          writeJson(res, 200, { ok: false, code: 'credentials-write-failed', message: error instanceof Error ? error.message : String(error) })
        }
      },
...
/**
 * Mount the plugin: settings section, bridge routes, the three providers, the
...
export function apply(ctx, config) {
  ...
  // --- 1. The providers. Nothing may precede these. ---------------------
  ...
  for (const id of DEFAULT_PRIORITY) {
    const provider = new SingleBackendProvider(id, backendFor(id))
  ...
  ctx.inject(['webServer', 'settings'], (sctx) => {
    ...
    const singleProbe = async (backend, query) => {
      ...
      return { query, backend: backend.id, totalResults: result.totalResults, sources: result.sources }
    }
    ...
    sctx.effect(() => {
      const disposers = makeBridgeRoutes({
        ...
        async adoptKey(key, toEnvFile) {
          if (toEnvFile === true) {
            const envPath = writeSkillEnvKey(key)
            return { stored: 'skill-env', path: envPath }
          }
  ...
  ctx.inject(['systemPrompt'], (sctx) => {
    ...
    refreshPrompt = () => {
      ...
      parts.push(
        '',
        `Reordering the chain, changing scoping, or saving a key all take effect on the next call — no restart. If ${TINYFISH_API_KEY_ENV} or the AnySearch key is missing, configure it in the dsh-hydrasearch settings card. Never claim the web is unreachable without reporting the provider error you saw.`,
      )
