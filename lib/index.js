...
/**
 * Mark a schema node editable at runtime, when the running schemastery can
 * express that.
 *
 * WHY THIS IS FEATURE-DETECTED RATHER THAN CALLED DIRECTLY
 * --------------------------------------------------------
 * `schemastery` grew a `.volatile()` marker in **3.18.3**. The packaged desktop
 * build this plugin still supports (DSH 0.1.6-alpha.2) ships **3.18.2**, where
 * `z.string().volatile` is `undefined` — calling it throws `TypeError:
 * ... .volatile is not a function` while the module is being evaluated, which
 * takes the whole plugin down at import time. Later DSH builds (0.1.7-alpha.1+)
 * require `^3.18.3` and DO have it.
 *
 * So the marker must be applied through this guard: on the old generation it is
 * a documented no-op, on the new one it is what makes the settings entry
 * visible and editable at all (see {@link makeConfig}).
 *
 * @param schema - any schemastery schema node.
 * @returns the same node, marked when the runtime supports marking.
 */
...
/**
 * Derive a registration-safe (unmarked) twin of a schema, when it carries the
 * volatile marker.
 *
 * WHY THE REGISTRATION-BASED GENERATIONS NEED THIS
 * ------------------------------------------------
 * A volatile-marked node returns a cordis REF, not a plain value, when it is
 * called. That is exactly what DSH 0.1.7's `SettingsForms` wants — it reads the
 * entry's `Config` for the form and the live reference for the values.
 *
 * The older services (`SettingsProvider.register` on the rc line, and
 * `settings.installSection` on 0.1.6-alpha.2) instead resolve the schema
 * THEMSELVES: `resolve()` calls `schema(mergeLayers(base, section))` and stores
 * the result as `registration.resolved`. Handing them the volatile `Config`
 * therefore stores a REF:
 *
 *   - `settings.get(ns).priority` is `undefined` (a ref has no own fields), so
 *     every config read collapses to "no config at all";
 *   - `describe()` hands the same ref to the card, which renders nothing;
 *   - the registration's `validate` hook receives the ref, so `validateConfig`
 *     sees no `tinyfish` / `anysearch` section and every malformed-value guard
 *     silently passes.
 *
 * The observable symptom is a working search tool with a dead configuration
 * card and unenforced validation — with no error anywhere.
 *
 * The twin is produced the way schemastery itself copies a node (`Schema(node)`,
 * the same idiom `.extra()` / `.required()` use): a fresh node sharing the
 * nested sub-schemas, with the root marker removed. Only the ROOT is unmarked —
 * the nested `tinyfish` / `anysearch` schemas were never marked, so no
 * descendant can turn into a ref. The source schema is left untouched, which
 * matters because `Config` is also the loader entry's schema.
 *
 * @param schema - the schema to register with a self-resolving settings service.
 * @returns an unmarked twin when the node is volatile-marked, else the node.
 */
...
/**
 * Read the live value out of a config node that may be a cordis volatile ref.
 *
 * A `volatile()` schema makes cordis hand `apply()` a **reference object** for
 * that node instead of a plain value: `config.priority` is then a ref whose
 * `.get()` returns the current section, and writing through it is what makes a
 * settings edit visible without a restart. On generations without volatile
 * support the node is a plain object/array, so both shapes must be accepted.
 *
 * Detection is structural (`[Symbol.for('cosmokit.volatile.write')] in value`)
 * rather than an import, because `@deepseek-ai/cosmokit` is a transitive
 * dependency of cordis and is not guaranteed to be resolvable from a profile
 * plugin. The symbol registry makes the check work across package copies.
 *
 * @param value - a volatile ref, or a plain value.
...
/**
 * The marker that identifies a cordis volatile reference. `Symbol.for` (rather
 * than a module import) so the test holds across duplicate cosmokit copies.
 */
const VOLATILE_WRITE = Symbol.for('cosmokit.volatile.write')

/**
 * Whether a config node is a cordis volatile reference.
 *
 * Structural rather than `instanceof`, for the same cross-copy reason as
 * {@link unwrapVolatile}: the running tree may hold several cosmokit copies.
 *
 * @param value - candidate node.
 * @returns whether the node implements the shared reference protocol.
 */
 ...
 *
 * WHY THE ROOT IS MARKED VOLATILE
 * -------------------------------
 * DSH 0.1.7-alpha.1 replaced `SettingsProvider` with `SettingsForms`, and the
 * new service only surfaces fields that sit under a `volatile` node:
 * `volatileForm(schema)` returns `undefined` unless some ancestor carries
 * `meta.volatile`, and an entry without a form is **omitted from `describe()`**.
 * A plugin that declares no volatile field therefore keeps working but loses its
 * entire configuration card — the symptom is a settings page that renders
 * nothing, with no error anywhere.
 *
 * Marking every editable leaf individually (what the official plugins do) would
 * mean touching ~35 fields and would silently exclude any field added later.
 * Marking the ROOT once covers the whole section and is self-maintaining: every
 * present and future field is editable, and `isVolatilePath` accepts every path,
 * which is exactly the semantics the settings card relies on (it addresses
 * fields by path with `mutate`, and the write path rejects any non-volatile
 * path).
 *
 * The nested `tinyfish` / `anysearch` sub-schemas are deliberately NOT marked.
 * The root marker already makes every descendant path volatile (verify with
 * `isVolatilePath`), and marking a sub-schema would change what calling it
 * returns: a volatile node yields a cordis REF rather than a plain value, so
 * `TinyfishConfig({})` — used throughout validation for defaults — would stop
 * being a plain section. Root-only marking keeps those calls plain.
 *
 * @returns the Config schema for this generation of the runtime.
 */
function makeConfig() {
  return volatile(
    z.object({
 ...
 * settings service is mounted, make the plugin's live config section the one
 * the settings card reads and writes; when the service goes away, fall back to
 * the composition entry.
 *
 * THREE GENERATIONS, TWO SHAPES
 * -----------------------------
 * `dsh-settings` has changed this contract twice, and the plugin must ride all
 * of them without a cross-version import (a static import of a name the running
 * tree lacks fails at ESM LINK time, which the loader reports as the opaque
 * `failed to import` — killing the whole plugin, not just its card):
 *
 *   1. **rc generation** — free function `installSettingsSection`, or the
 *      equivalent `settings.register(ns, schema, { base, validate })`.
 *   2. **0.1.6-alpha.2** — the free function is gone; the same wiring is the
 *      provider METHOD `settings.installSection(owner, ns, schema, entry, hooks)`.
 *   3. **0.1.7-alpha.1+** — `SettingsProvider` was replaced by `SettingsForms`,
 *      which has NEITHER method. It does not take registrations at all: forms
 *      are derived from the Loader entry's `Config` schema, keyed by the entry
 *      id (which is the namespace). Live values arrive through the volatile
 *      config reference that `apply()` already holds, so the only thing left to
 *      wire is the change notification.
 *
 * The feature detection is deliberately ordered by capability rather than by
 * version string, so an unreleased tree that keeps one of the older methods
 * still takes the corresponding path.
 *
...
export function installSettingsSection(ctx, ns, schema, entry, hooks) {
  ctx.inject(['settings'], (sctx) => {
    ...
    // Generations 1 and 2 resolve the schema themselves and STORE the result
    // (`registration.resolved = schema(mergeLayers(base, section))`). Handing
    // them a volatile-marked schema would store a cordis REF, so every read
    // through `settings.get(ns)` would collapse to `undefined` — see
    // {@link registrationSafeSchema}. Generation 3 never calls the schema, so it
    // must keep the marked original.
    //
    // Generation 2: the provider owns registration and cleanup.
    if (typeof service.installSection === 'function') {
      service.installSection(ctx, ns, registrationSafeSchema(schema), entry, hooks)
      return
    }

    // Generation 1: plain `register`, with the same effect contract the
    // shipped helper implements (swap the source back on disposal, then notify).
    if (typeof service.register === 'function') {
      const scope = service.register(settingsNamespace(ns), registrationSafeSchema(schema), options)
    ...
    // Generation 3: `SettingsForms`. Nothing is registered — the form is
    // generated from this plugin's `Config` (which must be volatile-marked, see
    // `makeConfig`) and the row's entry id IS the namespace. `setSource` is
    // intentionally NOT called: `apply()` already points it at the live config
    // reference, and overriding it here with a snapshot would pin the plugin to
    // the values captured at mount. Only the change notification is missing.
    const onChange = (updatedNs) => {
      if (String(updatedNs) !== settingsNamespace(ns)) return
      if (isUnloading(ctx)) return
      hooks.onChange()
    }
    sctx.on('settings/document-updated', onChange)
    hooks.onChange()
  })
}
 ...
 *
 * Built by {@link makeConfig} so the volatile marking can be feature-detected
 * against the running schemastery rather than assumed.
 */
export const Config = makeConfig()

/**
 * Resolve a raw config section into a plain, fully-defaulted object.
 *
 * REQUIRED because `Config` is volatile-marked at the root on runtimes that
 * support it, and schemastery returns a cordis REF (not a plain object) when a
 * volatile schema is called. Every consumer inside this plugin wants plain
 * values, so they must go through here rather than calling `Config` directly —
 * otherwise `cfg.priority` is `undefined` and reads like "no config at all".
 *
 * @param config - raw section from the loader, a volatile ref, or already-plain.
 * @returns a plain section with every schema default applied.
 */
...
export function apply(ctx, config) {
  ...
  //
  // On a runtime with volatile support (DSH 0.1.7-alpha.1+) the loader hands a
  // REFERENCE for a volatile-marked node, so `config` is a ref rather than a
  // plain object here. Keeping that reference — instead of a snapshot of it — is
  // what makes a settings edit visible without a restart: cordis writes through
  // the ref and `unwrapVolatile` re-reads it on every request. A snapshot would
  // pin the plugin to the values captured at mount.
  const raw = unwrapVolatile(config) ?? {}
  const resolved = resolveConfigObject(raw)

  // The live config section. On the registration-based generations
  // `installSettingsSection` swaps this thunk between the composition entry and
  // the resolved user scope. On the `SettingsForms` generation it stays as set
  // here and reads straight through the volatile ref, because that IS the live
  // authority the settings card writes to.
  let current = isVolatileRef(config)
    ? () => resolveConfigObject(config)
    : () => resolved
