 ...
 *    The key itself is SUPPLIED BY THE CALLER (the credential center); this
 *    module reads no key source of its own.
...
/** Environment variable the vendor CLI reads (diagnostic reference only). */
 ...
 *
 * Plugin-owned for the same reason as {@link TINYFISH_API_KEY_REF}: a reference
 * that collides with the conventional `ANYSEARCH_API_KEY` environment variable
 * would be shadowed by it, and the credential provider refuses to write a
 * shadowed reference — leaving the card unable to store or clear the key.
 */
export const ANYSEARCH_API_KEY_REF = 'HYDRASEARCH_ANYSEARCH_API_KEY'
...
export const ANYSEARCH_CLIENT_HEADER = 'dsh-hydrasearch/0.3.0'
