 ...
 * 2. Auth is a single `X-API-Key` header. The key is resolved per call from
 *    config → `$TINYFISH_API_KEY` → the TinyFish CLI's `~/.tinyfish/config.json`,
 *    so `tinyfish auth login` is picked up without a dsh restart.
...
/**
 * Read the key the TinyFish CLI persisted. `tinyfish auth login` writes
 * `~/.tinyfish/config.json` with an `api_key` field (mode 0600).
 *
 * @param home - home directory to read under; defaults to the current user's.
 * @returns the stored key, or `''` when absent/unreadable/malformed.
 */
export function readCliApiKey(home = os.homedir()) {
  try {
    const raw = JSON.parse(fs.readFileSync(path.join(home, '.tinyfish', 'config.json'), 'utf8'))
    if (raw === null || typeof raw !== 'object' || Array.isArray(raw)) return ''
    const key = raw.api_key
    return typeof key === 'string' && key.length > 0 ? key : ''
  } catch {
    // Absent, unreadable, or malformed config means "no key", never a crash: the
    // provider reports unavailability and the settings card explains the fix.
    return ''
  }
}

/**
 * Resolve the effective key. Precedence is explicit config, then the
 * environment, then the CLI's stored config — the order the CLI itself uses, so
 * `dsh` and `tinyfish` never disagree about which key is in force.
 *
 * @param configuredKey - key from the plugin's composition/settings config.
 * @param env - environment bag; injectable for tests.
 * @param home - home directory for the CLI-config fallback; injectable so a test
 *   can prove the fallback without depending on the developer's own
 *   `~/.tinyfish/config.json` (which made the check non-hermetic).
 * @returns the resolved key, or `''` when none is available.
 */
export function resolveTinyfishKey(configuredKey, env = process.env, home = os.homedir()) {
  if (typeof configuredKey === 'string' && configuredKey.trim().length > 0) return configuredKey.trim()
  const fromEnv = env[TINYFISH_API_KEY_ENV]
  if (typeof fromEnv === 'string' && fromEnv.trim().length > 0) return fromEnv.trim()
  return readCliApiKey(home)
}
